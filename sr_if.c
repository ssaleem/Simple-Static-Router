 ...
 * Scope: Global
 *
 * Given an interface  return the interface record or 0 if it doesn't
 * exist.
 *
 *---------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------- 
 * Method: sr_get_interface_fromMAC
 * Scope: Global
 *
 * Given an interface MAC  return the interface record or 0 if it doesn't
 * exist.
 *
 *---------------------------------------------------------------------*/

struct sr_if* sr_get_interface_fromMAC(struct sr_instance* sr, const char* mac)
{
    struct sr_if* if_walker = 0;
    ...
    while(if_walker)
    {
       if(!strncmp(if_walker->addr,mac,ETHER_ADDR_LEN))
        { return if_walker; }
        if_walker = if_walker->next;
    }

    return 0;
} /* -- sr_get_interface_fromMAC -- */
/*----------------------------------------------------------------------
* Testing if packet is destined to one of router interfaces. dst_ip is in NBO
*----------------------------------------------------------------------*/
int packet_for_me(struct sr_instance* sr, uint32_t dst_ip)
{
struct sr_if* if_walker = 0;
